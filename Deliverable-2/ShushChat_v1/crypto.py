...
# ── Constants ──────────────────────────────────────────────────────────────────
X25519_PUBKEY_LEN  = 32          # bytes — fixed by the X25519 spec
AES_KEY_LEN        = 32          # bytes — AES-256
GCM_NONCE_LEN      = 12         # bytes — 96-bit GCM nonce (NIST recommended)
GCM_TAG_LEN        = 16         # bytes — 128-bit GCM authentication tag
MAX_PLAINTEXT_LEN  = 128 * 1024 # 128 KB per message — prevents memory exhaustion
...
class Session:
    ...
    def decrypt(self, raw: bytes) -> str:
        ...
        if len(nonce) != GCM_NONCE_LEN:
            raise ValueError(
                f"Nonce must be {GCM_NONCE_LEN} bytes, got {len(nonce)}.")

        # Minimum ciphertext length = GCM_TAG_LEN (tag only, no plaintext)
        if len(ct) < GCM_TAG_LEN:
            raise ValueError(
                f"Ciphertext too short ({len(ct)} bytes).")

        # ── 4. Replay-attack prevention: reject duplicate nonces ──────
        nonce_key = bytes(nonce)
        with self._nonce_lock:
            if nonce_key in self._seen_nonces:
                raise ValueError(
                    "Duplicate nonce detected — possible replay attack.")
            # Record nonce; evict oldest when window is full
        ...
        # ── 5. AES-256-GCM decrypt (also verifies GCM tag) ───────────
        key    = bytes(self._key)
        aesgcm = AESGCM(key)
        try:
