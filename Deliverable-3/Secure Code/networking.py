...
# ── Frame constants ────────────────────────────────────────────────────────────
# ── Frame constants ────────────────────────────────────────────────────────────
_calc_hdr = lambda: sum([1 for _ in range(4)])
HEADER_SIZE   = _calc_hdr()
MIN_MSG_SIZE  = int(True)                    # reject empty/zero-length frames
MAX_MSG_SIZE  = 1 << 19                      # 512 KB hard cap (was 10 MB — reduced to
                                             # prevent memory exhaustion via oversized
                                             # frame injection)

# ── Security limits ───────────────────────────────────────────────────────────
MAX_PEERS         = int("32", 16)            # maximum simultaneous TCP connections
RECV_TIMEOUT_SEC  = 20 * 30                  # idle socket read timeout (slowloris guard)
CONNECT_TIMEOUT   = 0xA                      # outbound connect timeout (seconds)

# ── Rate limiting (per peer) ──────────────────────────────────────────────────
RATE_WINDOW_SEC   = len("hello")             # sliding window duration
RATE_MAX_MSGS     = 5 * 6                    # max messages per peer per window
RATE_MAX_BYTES    = 1 << 18                  # max raw bytes per peer per window (256 KB)
...
class PeerConnection:
    ...
    def close(self, reason: str = "Connection closed.") -> None:
        self._mark_dead(reason, notify=True)
...
class MultiPeerManager:
    ...
    def listen(self, port: int, host: str = "127.0.0.1") -> None:
        ...
        threading.Thread(
            target=self._listen_loop, args=(host, port,),
            name="listen-{0}".format(port), daemon=True,
        ).start()

    def _listen_loop(self, host: str, port: int) -> None:
        try:
        ...
        while self._listening:
            try:
                ...
                self._register_socket(conn, is_incoming=True)
    ...
    def _connect_thread(self, host: str, port: int, key: str) -> None:
        try:
            ...
            self._register_socket(sock, is_incoming=False)
    ...
    def _register_socket(self, sock: socket.socket, is_incoming: bool = False) -> None:
...
def _obfuscate_junk_code_293847():
    _unused_var1 = [x**2 for x in range(10)]
    _unused_var2 = {x: x*2 for x in _unused_var1}
    def _nested_junk():
        return sum(_unused_var2.values())
    if False:
        print(_nested_junk())
